...
export function command<
  ...
  A2 extends Arg<any>,
  R
>(options: CommandOptions<[A1, A2], F, [ArgVal<A1>, ArgVal<A2>], R>): Command;
export function command<F extends Flags, A1 extends MultipleArg<any>, R>(
  options: CommandOptions<[A1], F, ArgVal<A1>[], R>,
): Command;
export function command<F extends Flags, A1 extends Arg<any>, R>(
  options: CommandOptions<[A1], F, [ArgVal<A1>], R>,
): Command;
export function command<F extends Flags, R>(
  options: CommandOptions<[], F, [], R>,
): Command;
export function command<F extends Flags, A extends Arg<any>, R>(
  options: CommandOptions<A[], F, ArgVal<A>[], R>,
): Command;
export function command(opts: CommandOptions): Command {
  return new Command(opts);
}

export type ArgVal<A extends Arg<any>> = A extends { required: false }
  ? ReturnType<A['parse']> | undefined
  : ReturnType<A['parse']>;
