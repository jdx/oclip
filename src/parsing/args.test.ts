...
describe('required', () => {
  ...
  test('throws on missing arg', async () => {
    const cmd = command({
      args: [arg.required('FOO'), arg.required('BAR')],
      run: () => {},
    }).exec(['foo']);
  ...
  test('throws on missing arg and optional', async () => {
    const cmd = command({
      args: [arg.required('FOO'), arg.required('BAR'), arg.optional('BAZ')],
      run: () => {},
    }).exec(['foo']);
    await expect(cmd).rejects.toThrowError(/Missing 1 required arg:\nBAR/);
  });
  ...
  test('throws when extra arg', async () => {
    const cmd = command({ args: [arg('FOO')], run: () => {} }).exec([
      'foo',
      'bar',
    ]);
  ...
  test('name/options', () => {
    return command({
      ...
      run: ({ args, ctx }) => {
        ...
        expect(ctx.subject.args[0]).toMatchObject({
          name: 'FOO',
          description: 'DESC',
        });
      },
...
describe('default', () => {
  ...
  test('default multiple argument', () => {
    return command({
      args: [arg.multiple('C', { default: 123 })],
      run: ({ args }) => expect(args).toEqual([123]),
    }).exec([]);
  });
});
...
describe('choices', () => {
  ...
  test('picks a choice from a promise', () => {
    return command({
      args: [arg('A', { choices: async () => ['abc', '123'] })],
      run: ({ args }) => expect(args).toEqual(['123']),
    }).exec(['123']);
  });
  ...
  test('picks a wrong choice', () => {
    ...
    ).rejects.toThrowError(/Expected "124" to be one of:\nabc\n123/);
  });
});
