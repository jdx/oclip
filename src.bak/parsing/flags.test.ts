...
describe('bool', () => {
  ...
  test('sets true', async () => {
    ...
    expect(run).toBeCalledWith(
      expect.objectContaining({ flags: { foo: true } }),
    );
  });
  ...
  describe('multiple', () => {
    ...
    test('gets multiple', async () => {
      return command({
        ...
        run: ({ flags }) =>
          expect(flags).toMatchObject({ foo: ['123', '1234'] }),
  ...
  describe('required', () => {
    test('accepts input', () => {
      return command({
        ...
        run: ({ flags }) =>
          expect(flags).toMatchObject({ foo: '123', bar: '234' }),
    ...
    test('fails if missing', () => {
      ...
      ).rejects.toThrowError(/Missing required flag: --bar/);
    });
  });
  describe('choices', () => {
    test('accepts input', () => {
